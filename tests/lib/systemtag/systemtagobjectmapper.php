...
class TestSystemTagObjectMapper extends \Test\TestCase {
	...
	public function testAssignNonExistingTags() {
		$this->tagMapper->assignTags(1, 'testtype', ['unexist']);
	}

	public function testAssignNonExistingTagInArray() {
		$caught = false;
		try {
			$this->tagMapper->assignTags(1, 'testtype', ['unexist', $this->tag3->getId()]);
	...
	public function testUnassignNonExistingTags() {
		$this->tagMapper->unassignTags(1, 'testtype', ['unexist']);
	}

	public function testUnassignNonExistingTagsInArray() {
		$caught = false;
		try {
			$this->tagMapper->unassignTags(1, 'testtype', ['unexist', $this->tag1->getId()]);
		} catch (TagNotFoundException $e) {
			$caught = true;
		}
