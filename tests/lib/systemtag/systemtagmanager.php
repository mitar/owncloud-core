...
class TestSystemTagManager extends \Test\TestCase {
	...
	public function testGetNonExistingTagsById() {
		$tag1 = $this->tagManager->createTag('one', true, false);
		$this->tagManager->getTagsById([$tag1->getId(), 'unexist1', 'unexist2']);
	}
	...
	private function assertSameTagList($expectedTagList, $tagList) {
		$compareFunc = function($a, $b) {
			for ($i = 0; $i < count($a); $i++) {
				if ($a[$i] !== $b[$i]) {
					return strcmp((string)$a[$i], (string)$b[$i]);
				}
			}
			return 0;
		};

		usort($expectedTagList, $compareFunc);
		usort($tagList, $compareFunc);

		$this->assertSame($expectedTagList, $tagList);
	}
